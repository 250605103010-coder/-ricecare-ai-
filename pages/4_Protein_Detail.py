...
st.write(
    "InterPro helps identify known protein domains, families "
    "and functional regions."
)

interpro_rows = data_loader.get_analysis_for_protein(
    protein_id,
    "INTERPRO"
)
...
if (
    interpro_rows
    and interpro_rows[0].get("status")
    != "AWAITING_USER_INTERPRO_RESULTS"
):

    for row in interpro_rows:

        st.markdown(
            f"- **{row.get('detail_field_1', '')}** "
            f"({row.get('detail_field_2', '')}–"
            f"{row.get('detail_field_3', '')})"
        )

else:

    st.info(
        "No InterPro domain results have been added yet for this protein. "
        "Populate `data/protein_analysis.csv` "
        "(analysis_type = INTERPRO) with your actual domain output "
...
if has_real_analysis:
    ...
    st.success(
        "This rice protein is associated with the plant's "
        "defense/stress response. Sequence comparison identified "
        "related proteins and conserved regions, while domain analysis "
        "identified known functional regions that may help researchers "
    ...
    st.warning(
        "A complete molecular insight will be generated automatically "
        "once verified BLAST, MSA, and InterPro results are available "
        "for this protein. This ensures the conclusion is generated "
        "only from actual verified analysis results, never invented."
    )
